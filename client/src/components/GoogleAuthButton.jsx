import { useGoogleLogin } from '@react-oauth/google';
import axios from 'axios';
import { useAuth } from '../context/AuthContext';
import { useNavigate } from 'react-router-dom';

const API = import.meta.env.VITE_API_URL || 'http://localhost:3001';

const GoogleAuthButton = ({ label = 'Continuar con Google', onError }) => {
  const { login } = useAuth();
  const navigate = useNavigate();

  const handleGoogle = useGoogleLogin({
    onSuccess: async ({ access_token }) => {
      try {
        const { data } = await axios.post(`${API}/api/auth/google`, {
          accessToken: access_token,
        });
        login(data.token, data.usuario);
        navigate('/');
      } catch (err) {
        onError?.(err.response?.data?.error || 'Error al autenticar con Google');
      }
    },
    onError: () => onError?.('Error al conectar con Google'),
  });

  return (
    <button
      type="button"
      onClick={() => handleGoogle()}
      className="w-full flex items-center justify-center gap-3 border border-gray-300 bg-white text-gray-700 py-3 text-xs tracking-wider uppercase hover:bg-gray-50 transition-colors"
    >
      <svg width="16" height="16" viewBox="0 0 48 48">
        <path fill="#FFC107" d="M43.6 20.1H42V20H24v8h11.3C33.7 32.7 29.2 36 24 36c-6.6 0-12-5.4-12-12s5.4-12 12-12c3.1 0 5.8 1.2 7.9 3.1l5.7-5.7C34.1 6.5 29.3 4 24 4 12.9 4 4 12.9 4 24s8.9 20 20 20 20-8.9 20-20c0-1.3-.1-2.7-.4-3.9z"/>
        <path fill="#FF3D00" d="M6.3 14.7l6.6 4.8C14.5 16 19 12 24 12c3.1 0 5.8 1.2 7.9 3.1l5.7-5.7C34.1 6.5 29.3 4 24 4 16.3 4 9.7 8.4 6.3 14.7z"/>
        <path fill="#4CAF50" d="M24 44c5.2 0 9.9-2 13.4-5.2l-6.2-5.2C29.3 35.5 26.8 36 24 36c-5.2 0-9.6-3.3-11.3-8H6.1C9.5 35.5 16.3 44 24 44z"/>
        <path fill="#1976D2" d="M43.6 20.1H42V20H24v8h11.3c-.8 2.3-2.3 4.2-4.3 5.6l6.2 5.2C41.7 35.3 44 30 44 24c0-1.3-.1-2.7-.4-3.9z"/>
      </svg>
      {label}
    </button>
  );
};

export default GoogleAuthButton;
